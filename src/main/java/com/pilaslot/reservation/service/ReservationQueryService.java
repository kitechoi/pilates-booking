...
public class ReservationQueryService {
    ...
    public MyReservationListResponse getMyReservations(
        ...
        ReservationStatus status = parseStatus(statusValue);
        ...
        List<MyReservationResponse> responses = reservations.stream()
                .map(reservation -> MyReservationResponse.from(reservation, now))
                .toList();
    ...
    private ReservationStatus parseStatus(String statusValue) {
        if (statusValue == null) {
            return null;
        }
        try {
